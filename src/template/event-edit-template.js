...
// почему не реагирует на пустые строки в BLANK__POINT? 
...
function createDesinationListOption (destination) {
  return(
 ...
 function createEditTemplate(data){
  ...
  const {destination, type, dateFrom,dateTo,eventPrice,eventDate} = data;
  const eventTypeItem = createEventTypeItemTemplate(type);
  const destinationOption = createDesinationListOption (destination);
  console.log(data)
